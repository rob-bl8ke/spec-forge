...
**Validation:**
- Spec Forge will check that the `repoPath` you provide exists on disk at startup.
- If the directory does not exist, you will get a clear error and the workflow will not run.
- This prevents confusing errors later (such as AI calls failing with cryptic ENOENT or empty output).

**What you must do:**
- Make sure the directory you specify in `repoPath` actually exists before running any workflows.
- Example: If your project config says `repoPath: ../communication-service`, then `../communication-service` (relative to your `spec-forge` folder) **must exist**.
- If you move or rename your codebase, update `repoPath` accordingly.

**Troubleshooting:**
- If you see an error like:
  > Invalid config at projects/comm-service.yaml: repoPath does not exist on disk: C:/Code/communication-service
  then create the missing directory or fix the path in your project YAML.

**Summary:**
- Always check your `repoPath` is correct and exists before running or initializing a project.
- This validation ensures Spec Forge can safely run all workflows and spawn AI tools in the right context.
