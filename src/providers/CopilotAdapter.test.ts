...
test("CopilotAdapter isAvailable returns true when gh copilot is accessible", async () => {
  const child = createFakeChild();
  const adapter = new CopilotAdapter(() => child);
...
test("CopilotAdapter isAvailable returns false when spawn errors", async () => {
  const child = createFakeChild();
  const adapter = new CopilotAdapter(() => child);
...
test("generate sends prompt via stdin and returns populated response on success", async () => {
  ...
  const adapter = new CopilotAdapter((_, __, options) => {
